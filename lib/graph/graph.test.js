...
describe('Graph', () => {
  ...
  it('Should create directed weighted graph', () => {
    ...
    const data = graph.getData();
    should(data[0]).be.containEql('1', '2');
    should(data[0]).have.length(2);
    should(data[1]).be.containEql('2');
    should(data[1]).have.length(1);
    should(data[2]).have.length(0);
    should(data[3]).be.containEql('0');
    should(data[3]).have.length(1);
    should(data[4]).be.containEql('0');
    should(data[4]).have.length(1);
    should(graph.getEdgeWeight('0', '1')).be.eql(5);
    should(graph.getEdgeWeight('1', '2')).be.eql(6);
    should(graph.getEdgeWeight('0', '2')).be.eql(3);
    should(graph.getEdgeWeight('4', '0')).be.eql(-2);
    should(graph.getEdgeWeight('3', '0')).be.eql(2);
  })
