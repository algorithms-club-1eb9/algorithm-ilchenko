...
describe('Graph', () => {
  ...
  it.only('Should create directed weighted graph', () => {
    const graph = new algo.Graph({
      directed: true,
      weighted: true
    });
    ...
    const data = graph.getData();
    should(data[0]).be.containEql(['1', 5], ['2', 3]);
    should(data[0]).have.length(2);
    should(data[1]).be.containEql(['2', 6]);
    should(data[1]).have.length(1);
    should(data[2]).have.length(0);
    should(data[3]).be.containEql(['0', 2]);
    should(data[3]).have.length(1);
    should(data[4]).be.containEql(['0', -2]);
    should(data[4]).have.length(1);
  })
